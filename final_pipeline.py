...
These results are preliminary holdout benchmarks only. They are not used for final model selection or hyperparameter tuning. The same held-out test set is used for final reporting, while model tuning is performed only on the training set using cross-validation.
...
results_df.to_csv(OUTPUTS_DIR / "hamza_tuning_results.csv", index=False)

joblib.dump(rf_search_restricted.best_estimator_,  MODELS_DIR / "best_rf_restricted.pkl")
joblib.dump(gbm_search_restricted.best_estimator_, MODELS_DIR / "best_gbm_restricted.pkl")
joblib.dump(rf_search_full.best_estimator_,        MODELS_DIR / "best_rf_full.pkl")
joblib.dump(gbm_search_full.best_estimator_,       MODELS_DIR / "best_gbm_full.pkl")
print("All 4 tuned pipelines saved")

# Generate test data files for Noor
X_test[restricted_features].to_csv(OUTPUTS_DIR / "X_test_restricted.csv", index=False)
X_test[full_features].to_csv(OUTPUTS_DIR / "X_test_full.csv", index=False)
...
This section is the final evaluation and interpretation section of the group project. It uses the final tuned models and test data from Hamza’s tuning notebook to compare model performance, select the final model, interpret the results, and discuss limitations.
