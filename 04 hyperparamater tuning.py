...
This section performs hyperparameter tuning on the best-performing baseline models (Random Forest and Gradient Boosting) that we found in Step 3 (Haya). Tuning is done using RandomizedSearchCV with 5-fold stratified cross-validation, applied only on the training set. The test set is never used during tuning.
"""
